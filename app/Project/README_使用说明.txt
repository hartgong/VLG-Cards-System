...
python Scripts/refresh_inventory.py Data/card_data_warehouse_V3.xlsx Data/card_data_warehouse_V3_after_refresh.xlsx


输出文件：

text
Data/card_data_warehouse_V3_after_refresh.xlsx
...
## V3 库存刷新方式：原地修改同一个 Excel 文件

V3 现在采用“原地刷新”方式，不再要求输入文件和输出文件两个路径。

你只需要修改 `Data/card_data_warehouse_V3.xlsx` 里的 `Purchase_Inbound`，保存并关闭 Excel，然后运行：


python Scripts/refresh_inventory.py Data/card_data_warehouse_V3.xlsx


脚本会直接在这个 Excel 文件内刷新：

1. `Inventory_Ledger`：根据 `Purchase_Inbound` 重建 `Purchase In` 入库流水；根据 `Order_Line` 重建 `Sales Out` 销售扣库流水。
2. `Current_Inventory`：根据 `Inventory_Ledger` 汇总当前库存。
3. `Exception_Log`：刷新库存相关异常。
...
注意：运行脚本前必须关闭正在打开的 Excel 文件，否则 Windows 可能会阻止保存。
